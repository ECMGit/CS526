...
public class ArtworkManagement {
    ...
    public static void main(String[] args) throws IOException {
        ...
        SinglyLinkedList<Person> artistList = new SinglyLinkedList<>();
        //test data
        Person shen = new Person("Shen", 1994);
        Person samba = new Person("Samba", 1992);
        Person elder = new Person("ZeminJiang", 1926);
        artistList.addFirst(shen);
        artistList.addLast(samba);
        artistList.addLast(elder);
        ...
        while (flag == 0) {
            ...
            switch (a) {
                ...
                case "1":
                    ...
                    System.out.print("Please enter a artist name: ");
                    String artist_name = inputinfo.next();
                    Person artist_add = ArtworkManagement.addArtist(artist_name, artistList, inputinfo);
                    switch (type) {

                        case "painting":
                            if (searchPainting(id, paintingSinglyLinkedList) == null) {

                                ArtworkManagement.addPainting(id, artist_add, paintingSinglyLinkedList, inputinfo);
                            } else {
                                System.out.println("The painting id has already existed!");
                            }
                            flag = 0;
                            break;
                        case "sculpture":
                            if (searchSculpture(id, sculptureSinglyLinkedList) == null) {
                                ArtworkManagement.addSculpture(id, artist_add, sculptureSinglyLinkedList, inputinfo);
                            } else {
                                System.out.println("The sculpture id has already existed!");
                            }
                            flag = 0;
                            break;
                        default:
     ...
     * @param inputinfo
     */
    public static void addPainting(String id, Person artist,SinglyLinkedList<Painting> paintingSinglyLinkedList, Scanner inputinfo) {
    ...
    public static void addSculpture(String id, Person artist,SinglyLinkedList<Sculpture> sculptureSinglyLinkedList, Scanner inputinfo) {
        System.out.print("year: ");
        int year = inputinfo.nextInt();
        if (year > 0) {
            System.out.print("location: ");
    ...
    /**
     * add element into artistList
     */
    public static Person addArtist(String artist_name, SinglyLinkedList<Person> artistList, Scanner inputinfo) {
        Person existed_artist = null;
        for (int m = 0; m < artistList.size(); m++) {
            if (artist_name.equals(artistList.getElements(m, artistList).getName())) {
                existed_artist = artistList.getElements(m, artistList);
                break;
            }
        }
        if (existed_artist == null) {
            System.out.print("And artist's birth year: ");
            int artist_year = inputinfo.nextInt();
            Person new_artist = new Person(artist_name, artist_year);
            artistList.addLast(new_artist);
            return new_artist;
        } else {
    ...
    /**
     * clone a new paintinglist, a simple method modified from clone() method;
     *
     * @param paintingsList
     * @return
     */
    public static SinglyLinkedList<Painting> copyPaintingsList(SinglyLinkedList<Painting> paintingsList) {
        SinglyLinkedList<Painting> newp = null;
        try {
            newp = paintingsList.clone();
